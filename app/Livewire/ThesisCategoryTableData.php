...
class ThesisCategoryTableData extends Component
{
    use WithPagination;
    ...
    public function delete($id)
    {
     ...
     public function save()
     {
         try {
             $validated = $this->validate([
                 'newName' => 'required|string|max:255|unique:thesis_categories,name',
     ...
     public function update($id) {
        try {
            $validated = $this->validate([
                'name' => 'required|string|max:255|unique:thesis_categories,name,' . $id,
    ...
    public function render(){
        ...
        return view('livewire.thesis-category-table-data', [
            'items' => $items,
        ]);
    }
}
