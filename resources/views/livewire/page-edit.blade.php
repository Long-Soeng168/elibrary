        ...
        <form class="w-full" action="{{ route('admin.items.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid gap-5 mb-5 lg:grid-cols-2 lg:gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="image_link" class="block w-full" type="text" name="image_link" wire:model='image_url'
                        placeholder="Image Link" />
                    <x-input-error :messages="$errors->get('image_link')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="video_link" :value="__('Video Link')" />
                    <x-text-input id="video_link" class="block w-full" type="text" name="video_link" wire:model='video_url'
            ...
            <div class="grid lg:grid-cols-2 lg:gap-6">
                ...
                <div class="relative w-full mb-5 group">
                    <x-input-label for="protected_area_id" :value="__('Main Menu')" />
                    <div class="flex flex-1 gap-1 mt-1">
                        <div class="flex justify-start flex-1">
                            <x-select-option wire:model.live='protected_area_id' id="protected_area_id"
                                name="protected_area_id" class="category-select">
                                <option wire:key='category' value="">Select Main Menu...</option>
                                @forelse ($categories as $category)
                                    <option wire:key='{{ $category->id }}' value="{{ $category->id }}">
                                        {{ $category->name }}
                                    </option>
                                @empty
                                    <option wire:key='nocateogry' value=""> --No Main Menu--</option>
                                @endforelse
                            </x-select-option>
                        </div>

                    </div>
                    <x-input-error :messages="$errors->get('protected_area_id')" class="mt-2" />
                </div>
                {{-- End Category Select --}}

                {{-- Start Sub-Category Select --}}
                <div class="relative w-full mb-5 group">
                    <x-input-label for="area_id" :value="__('Sub-Menu')" />
                    <div class="flex flex-1 gap-1 mt-1">
                        <div class="flex justify-start flex-1">
                            <x-select-option wire:model.live='area_id'
                                id="area_id" name="category_id" class="sub-category-select">
                                <option wire:key='sub-category' value="">
                                    {{ $protected_area_id ? 'Select Sub-Menu...' : 'Select Main First' }}
                                </option>
                                @forelse ($subCategories as $subCategory)
                                    <option wire:key='{{ $subCategory->id }}' value="{{ $subCategory->id }}">
                                        {{ $subCategory->name }}</option>
                                @empty
                                    <option wire:key='nosub-category' value="">--No Sub-Menu--</option>
                                @endforelse
                            </x-select-option>
                        </div>
        ...
        function initSelect2() {
            $(document).ready(function() {
                ...
                $('.category-select').on('change', function(event) {
                    let data = $(this).val();
                    @this.set('protected_area_id', data);
                });
