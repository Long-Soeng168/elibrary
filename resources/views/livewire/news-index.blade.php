            ...
            <div class="max-w-screen-xl mx-auto mt-6">
                ...
                <div
                    ...
                    @forelse ($items as $index => $item)
                        ...
                        <a wire:key="{{ $item->id }}-{{ $index }}" class="block group" href="{{ url('bulletins/'.$item->id) }}">
                            <div class="w-full overflow-hidden bg-gray-100 border rounded-md dark:bg-neutral-800">
                                @if ($item->image)
                                        <img class="w-full aspect-[{{ env('JOURNAL_ASPECT') }}] group-hover:scale-110 transition-transform duration-500 ease-in-out object-cover rounded-md"
                                        src="{{ asset('assets/images/news/thumb/'.$item->image) }}"
                                        alt="Image Description" />
                                @else
                                    <img class="w-full aspect-[{{ env('JOURNAL_ASPECT') }}] p-6 group-hover:scale-110 transition-transform duration-500 dark:bg-gray-300 ease-in-out object-contain rounded-md"
