                    ...
                    @forelse ($items as $index => $item)
                        ...
                        <a wire:key="{{ $item->id }}-{{ $index }}" class="block group" href="{{ url('publications/'.$item->id) }}">
                            <div class="w-full overflow-hidden bg-gray-100 border rounded-md dark:bg-neutral-800">
                                ...
                                @else
                                    <img class="w-full aspect-[{{ env('EPUB_ASPECT') }}] p-6 group-hover:scale-110 transition-transform duration-500 dark:bg-gray-300 ease-in-out object-contain rounded-md"
