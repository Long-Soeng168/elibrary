...
@section('content')
        ...
        <div class="min-[1000px]:grid grid-cols-12 gap-4 px-2">
            <div class="flex flex-col items-center col-span-5 mb-6">
                <div class="flex flex-col w-full gap-2 ">
                    ...
                    <a href="{{ asset('assets/images/audios/'.$item->image) }}" class="glightbox">
                        <img class="w-full border rounded-md shadow cursor-pointer"
            ...
            <div class="col-span-7">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    @if ($item->audioType?->name)
                        <div class="flex nowrap">
                            <p class="w-[123px] uppercase tracking-wide text-sm text-gray-500 dark:text-gray-300 font-semibold border-r border-gray-600 dark:border-gray-300 pr-5 mr-5 flex-shrink-0">
                                {{ __('messages.type') }}
                            </p>
                            <p class="text-sm text-gray-600 dark:text-gray-200">
                                {{ app()->getLocale() == 'kh' ? $item->audioType?->name_kh : $item->audioType?->name }}
                            </p>
                        </div>
                    @endif

                    @if ($item->audioCategory?->name)
                        <div class="flex nowrap">
                            <p class="w-[123px] uppercase tracking-wide text-sm text-gray-500 dark:text-gray-300 font-semibold border-r border-gray-600 dark:border-gray-300 pr-5 mr-5 flex-shrink-0">
                                {{ __('messages.category') }}
                            </p>
                            <p class="text-sm text-gray-600 dark:text-gray-200">
                                @if (app()->getLocale() == 'kh' && $item->audioCategory?->name_kh)
                                    {{ $item->audioCategory?->name_kh }}
                                @else
                                    {{ $item->audioCategory?->name }}
                                @endif

                                @if (app()->getLocale() == 'kh' && $item->audioSubCategory?->name_kh)
                                / {{ $item->audioSubCategory?->name_kh }}
                                @elseif($item->audioSubCategory?->name)
                                / {{  $item->audioSubCategory?->name }}
                                @endif
    ...
    <div class="max-w-screen-xl px-2 mx-auto mt-6">
        ...
        @empty
            <p class="dark:text-white">No Related Item...</p>
        @endforelse
        </div>
        <!-- End Card Grid -->
    </div>
