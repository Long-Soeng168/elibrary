...
@section('content')
    ...
    <!-- Detail -->
    <!--$ipRanges = [-->
    <!--            // '192.168.1.0/24',   // IP range for 192.168.1.0 - 192.168.1.255-->
    <!--            // '10.0.0.0/8',       // IP range for 10.0.0.0 - 10.255.255.255-->
    <!--            '203.144.68.205/32' // Specific IP address-->
    <!--        ];-->
    <div class="max-w-screen-xl px-2 mx-auto mt-6 lg:px-0">
        ...
        <!--@if($ipInRange)-->
            <!-- Render the element for users within any of the specified IP ranges -->
        <!--    <p>You are within the specified IP range.</p>-->
        <!--@endif-->
        
        <!-- The rest of your page content -->
        <!--<p>This is the rest of the page that should always load.</p>-->
        ...
        <div class="min-[800px]:grid grid-cols-12 gap-4">
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        @foreach ($multi_images as $index => $image)
                            @if ($index < 3 || count($multi_images) == 4)
                                <a href="{{ asset('assets/images/articles/thumb/' . $image->image) }}"
                                    class="glightbox">
                            ...
                            @else
                                <a href="{{ asset('assets/images/articles/' . $image->image) }}" class="glightbox">
                        ...
                        <div class="flex-1">
                            ...
                            <a
                                @if (!$item->can_read && !auth()->check())
                                 href="{{ route('client.login', ['path' => 'articles-'.$item->id]) }}"
                                @else
            ...
            <div class="col-span-7 lg:col-span-8">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    @if ($item->articleType?->name)
                        <div class="flex nowrap">
                            <p class="w-[123px] uppercase tracking-wide text-sm text-gray-500 dark:text-gray-300 font-semibold border-r border-gray-600 dark:border-gray-300 pr-5 mr-5 flex-shrink-0">
                                {{ __('messages.type') }}
                            </p>
                            <p class="text-sm text-gray-600 dark:text-gray-200">
                                {{ app()->getLocale() == 'kh' ? $item->articleType?->name_kh : $item->articleType?->name }}
                            </p>
                        </div>
                    @endif
